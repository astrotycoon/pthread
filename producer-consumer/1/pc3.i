...
# 1 "/usr/lib/gcc/i686-linux-gnu/4.9/include/stddef.h" 1 3 4
# 324 "/usr/lib/gcc/i686-linux-gnu/4.9/include/stddef.h" 3 4
...
# 95 "/usr/include/i386-linux-gnu/bits/sched.h" 3 4
...
# 6 "pc3.c" 2
# 1 "/usr/include/i386-linux-gnu/sys/queue.h" 1 3 4
# 7 "pc3.c" 2
# 30 "pc3.c"
struct _Data {
 int value;
 struct { struct _Data *tqe_next; struct _Data * *tqe_prev; } entry;
};

struct __workq {
 struct workq { struct _Data *tqh_first; struct _Data * *tqh_last; } head;
 int cnt;
 pthread_mutex_t lock;
 pthread_cond_t got_consumer_cond;
 pthread_cond_t got_producer_cond;
};

static void queue_add(struct __workq *pworkq)
{
 do { int result; if ((result = (pthread_mutex_lock(&pworkq->lock))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_mutex_lock(&pworkq->lock)"); exit(1); } } while ( 0);
 while (pworkq->cnt == (1000)) {
  printf("---------------------------->full.\n");

  do { int result; if ((result = (pthread_cond_wait(&pworkq->got_consumer_cond, &pworkq->lock))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_cond_wait(&pworkq->got_consumer_cond, &pworkq->lock)"); exit(1); } } while ( 0);
 }
 struct _Data *pdata = (struct _Data *)calloc(1, sizeof(struct _Data));
 pdata->value = rand() % 1000 + 1;
 do { if (((pdata)->entry.tqe_next = (&pworkq->head)->tqh_first) != ((void *)0)) (&pworkq->head)->tqh_first->entry.tqe_prev = &(pdata)->entry.tqe_next; else (&pworkq->head)->tqh_last = &(pdata)->entry.tqe_next; (&pworkq->head)->tqh_first = (pdata); (pdata)->entry.tqe_prev = &(&pworkq->head)->tqh_first; } while ( 0);
 ++pworkq->cnt;
 ...
 do { int result; if ((result = (pthread_mutex_unlock(&pworkq->lock))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_mutex_unlock(&pworkq->lock)"); exit(1); } } while ( 0);
 do { int result; if ((result = (pthread_cond_broadcast(&pworkq->got_producer_cond))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_cond_broadcast(&pworkq->got_producer_cond)"); exit(1); } } while ( 0);
}
...
static void queue_remove(struct __workq *pworkq)
{
 ...
 while (pworkq->cnt == 0) {
  ...
  do { int result; if ((result = (pthread_cond_wait(&pworkq->got_producer_cond, &pworkq->lock))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_cond_wait(&pworkq->got_producer_cond, &pworkq->lock)"); exit(1); } } while ( 0);
 }
 struct _Data *pdata = ((&pworkq->head)->tqh_first);
 do { if (((pdata)->entry.tqe_next) != ((void *)0)) (pdata)->entry.tqe_next->entry.tqe_prev = (pdata)->entry.tqe_prev; else (&pworkq->head)->tqh_last = (pdata)->entry.tqe_prev; *(pdata)->entry.tqe_prev = (pdata)->entry.tqe_next; } while ( 0);
 --pworkq->cnt;
 ...
 do { int result; if ((result = (pthread_mutex_unlock(&pworkq->lock))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_mutex_unlock(&pworkq->lock)"); exit(1); } } while ( 0);
 do { int result; if ((result = (pthread_cond_broadcast(&pworkq->got_consumer_cond))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_cond_broadcast(&pworkq->got_consumer_cond)"); exit(1); } } while ( 0);
 free(pdata);
}
...
begin:
 srand(time(((void *)0)));
 WorkQ.cnt = 0;
 do { int result; if ((result = (pthread_mutex_init(&WorkQ.lock, ((void *)0)))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_mutex_init(&WorkQ.lock, NULL)"); exit(1); } } while ( 0);
 do { int result; if ((result = (pthread_cond_init(&WorkQ.got_consumer_cond, ((void *)0)))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_cond_init(&WorkQ.got_consumer_cond, NULL)"); exit(1); } } while ( 0);
 do { int result; if ((result = (pthread_cond_init(&WorkQ.got_producer_cond, ((void *)0)))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_cond_init(&WorkQ.got_producer_cond, NULL)"); exit(1); } } while ( 0);
 do { (&WorkQ.head)->tqh_first = ((void *)0); (&WorkQ.head)->tqh_last = &(&WorkQ.head)->tqh_first; } while ( 0);


 for (i = 0; i < (3); i++) {
  do { int result; if ((result = (pthread_create(&cth[i], ((void *)0), consumer, &WorkQ))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_create(&cth[i], NULL, consumer, &WorkQ)"); exit(1); } } while ( 0);
 }
 sleep(2);

 for (i = 0; i < (5); i++) {
  do { int result; if ((result = (pthread_create(&pth[i], ((void *)0), producer, &WorkQ))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_create(&pth[i], NULL, producer, &WorkQ)"); exit(1); } } while ( 0);
 }

end:
 for (i = 0; i < (5); i++) {
  do { int result; if ((result = (pthread_join(pth[i], ((void *)0)))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_join(pth[i], NULL)"); exit(1); } } while ( 0);
 }
 for (i = 0; i < (3); i++) {
  do { int result; if ((result = (pthread_join(cth[i], ((void *)0)))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_join(cth[i], NULL)"); exit(1); } } while ( 0);
 }

 do { int result; if ((result = (pthread_mutex_destroy(&WorkQ.lock))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_mutex_destroy(&WorkQ.lock)"); exit(1); } } while ( 0);
 do { int result; if ((result = (pthread_cond_destroy(&WorkQ.got_consumer_cond))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_cond_destroy(&WorkQ.got_consumer_cond)"); exit(1); } } while ( 0);
 do { int result; if ((result = (pthread_cond_destroy(&WorkQ.got_producer_cond))) != 0) { fprintf(stderr, "FATAL: %s (%s)\n", strerror(result), "pthread_cond_destroy(&WorkQ.got_producer_cond)"); exit(1); } } while ( 0);

 do { int result; if ((result = (open("xxx", O_RDONLY, 0))) == -1) { fprintf(stderr, "FATAL: %s (%s)(%s)\n", strerror(result), "open(\"xxx\", O_RDONLY, 0)", "XXX"); exit(1); } } while ( 0);
