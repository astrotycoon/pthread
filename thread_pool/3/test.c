...
#define QUEUE_MAX_NUM		(100)	/* 规定队列最多拥有100个队列元素 */
...
begin:
	...
	thr_pool_init();
	thr_pool_create(PRODUCER_THREAD_NUM + CONSUMER_THREAD_NUM, THR_POOL_MAX_THREADS, 0, NULL, &phl);
	for (i = 0; i < CONSUMER_THREAD_NUM; i++) {
		thr_pool_queue(phl, 0, 0, consumer, &WorkQ);
	}
	sleep(3);
	for (i = 0; i < PRODUCER_THREAD_NUM; i++) {
		thr_pool_queue(phl, 0, 0, producer, &WorkQ);
	}
	while (1) {
		usleep(1000);
	}
end:
	thr_pool_destroy(phl);
	thr_pool_free();
	exit(EXIT_SUCCESS);
}
